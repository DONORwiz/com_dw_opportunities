 ...
 */
class Dw_opportunitiesModelDwOpportunityForm extends JModelForm
{
	...
	public function &getData($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
                ...
                $id = $table->id;
                $canEdit = $user->authorise('core.edit', 'com_dw_opportunities') || $user->authorise('core.create', 'com_dw_opportunities');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_dw_opportunities.opportunity', 'dwopportunityform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		$data = JFactory::getApplication()->getUserState('com_dw_opportunities.edit.opportunity.data', array());
        if (empty($data)) {
            $data = $this->getData();
        }
	...
	{
		$id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('opportunity.id');
        $state = (!empty($data['state'])) ? 1 : 0;
        $user = JFactory::getUser();

        if($id) {
            //Check the user can edit this item
            $authorised = $user->authorise('core.edit', 'com_dw_opportunities') || $authorised = $user->authorise('core.edit.own', 'com_dw_opportunities');
            if($user->authorise('core.edit.state', 'com_dw_opportunities') !== true && $state == 1){ //The user cannot edit the state of the item.
                $data['state'] = 0;
            }
        } else {
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('opportunity.id');
        if(JFactory::getUser()->authorise('core.delete', 'com_dw_opportunities') !== true){
            JError::raiseError(403, JText::_('JERROR_ALERTNOAUTHOR'));
            return false;
        }
        $table = $this->getTable();
